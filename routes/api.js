...
const { postFilm, getFilms, getFilmByID, deleteFilm, updateFilm } = require("../db");
...
module.exports = (app) => {
    ...
    app.delete(`${API_PATH}/films/:id`, async (req, res)=>{
        const id = req.params.id;
        if (id) { 
            const resp = await deleteFilm(id);
            return res.json(resp);
        }
        res.status(400).send({ reason: "No film delete." });
    })

    app.put(`${API_PATH}/films/:id`, async (req, res)=>{
        const id = req.params.id;
        const title = JSON.parse(req.body.film).title;
        if (title) { 
            const resp = await updateFilm(id, title);
            return res.json(resp);
        }
        res.status(400).send({ reason: "No film update." });
    })
}